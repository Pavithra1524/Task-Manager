...
export default function UserList({ team, setTeam }) {
  ...
  const getDisplayName = (user) => {
    return user.name || user.first_name || user.username || user.email || "Unknown";
  };
        ...
        <div className='relative mt-1'>
          <Listbox.Button className='relative w-full cursor-default rounded bg-white pl-3 pr-10 text-left px-3 py-2.5 2xl:py-3 border border-gray-300 dark:border-gray-600 sm:text-sm'>
            <span className='block truncate'>
              {selectedUsers?.map(getDisplayName).join(", ") || "Select team members"}
            </span>
                  ...
                  {({ selected }) => (
                    <>
                      <div className={`flex items-center gap-2 truncate ${selected ? "font-medium" : "font-normal"}`}>
                        ...
                        <span className="ml-2 text-xs text-gray-500">{user.title} / {user.role}</span>
                      </div>
                      {selected ? (
